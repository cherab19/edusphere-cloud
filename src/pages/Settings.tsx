...
import { School, Phone, Mail, MapPin, Save, Check, Crown, Upload, Image, User } from "lucide-react";
...
const Settings = () => {
  const { schoolId, profile, user, roles } = useAuth();
  ...
  useEffect(() => {
    if (school) {
      setForm({ name: school.name, email: school.email, phone: school.phone ?? "", address: school.address ?? "" });
    }
  }, [school]);
  ...
  const updateSchool = useMutation({
    mutationFn: async () => {
      const { error } = await supabase
        .from("schools")
        .update({ name: form.name, email: form.email, phone: form.phone || null, address: form.address || null })
        .eq("id", schoolId!);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { data: { publicUrl } } = supabase.storage.from("school-logos").getPublicUrl(filePath);
  ...
  const updatePlan = useMutation({
    mutationFn: async (plan: string) => {
      ...
      const { error } = await supabase.from("subscriptions").update({ plan: plan as any }).eq("id", subscription.id);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-3xl">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Manage your profile, school information and subscription</p>
        </div>

        <Tabs defaultValue="profile">
          <TabsList className="rounded-xl">
            <TabsTrigger value="profile" className="rounded-lg">My Profile</TabsTrigger>
            {isAdmin && <TabsTrigger value="school" className="rounded-lg">School Info</TabsTrigger>}
            {isAdmin && <TabsTrigger value="subscription" className="rounded-lg">Subscription</TabsTrigger>}
          </TabsList>
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input className="pl-10 h-11 rounded-xl" value={profileForm.full_name} onChange={(e) => setProfileForm({ ...profileForm, full_name: e.target.value })} />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label>Email</Label>
                  <div className="relative">
                    ...
                    <Input className="pl-10 h-11 rounded-xl" value={user?.email ?? ""} disabled />
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input className="pl-10 h-11 rounded-xl" value={profileForm.phone} onChange={(e) => setProfileForm({ ...profileForm, phone: e.target.value })} />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label>Role</Label>
                ...
                <Button className="rounded-xl gap-2" onClick={() => updateProfile.mutate()} disabled={updateProfile.isPending || !profileForm.full_name}>
                  {updateProfile.isPending ? "Saving…" : <><Save className="w-4 h-4" /> Save Profile</>}
                </Button>
              </div>
            </Card>
          </TabsContent>
